  ...
  <div className="w-full">
    <div className="grid grid-cols-2 gap-1 mt-1 mb-1 px-1">
      {paymentButtons.map((btn, i) => (
        <button
          key={i}
          className={`w-full px-2 py-2 rounded text-[13px] font-bold shadow transition-all ${btn.color} whitespace-nowrap flex items-center justify-center gap-1 ${btn.doubleHeight ? 'row-span-2 h-[104px] min-h-[104px]' : ''}`}
          onClick={btn.onClick}
